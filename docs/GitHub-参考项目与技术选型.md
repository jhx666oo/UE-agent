...
## 9. Vibe Coding 与设计系统约束

项目采用“详细规范 + Agent 指令 + 自动检查”的约束方式。AI 或人工开发不得根据单次提示重新选择组件库、颜色、圆角和页面结构。

- 详细规则以 [`docs/standards/`](standards/00-规范总则.md) 为唯一规范来源。
- 根目录 `AGENTS.md` 保存跨工具核心规则。
- GitHub Copilot 和 Cursor 使用各自的短规则文件，并引用同一套规范。
- 共享组件统一放入 `packages/ui`，业务页面不得复制基础组件。
- shadcn/ui 初始化风格固定为 `radix-nova`，Tailwind CSS v4 使用 CSS Variables。
- 首版只支持浅色主题，未经设计和测试不得增加暗色主题。
- 视觉和交互完成标准以 [`06-前端验收检查表.md`](standards/06-前端验收检查表.md) 为准。
